  ...
  return (
    <div className="h-full w-full bg-black flex flex-col relative overflow-hidden">
      {/* 
          ...
          Uses env(safe-area-inset-top) + extra padding to avoid the center camera punch-hole 
      */}
      <header className="px-8 flex justify-between items-center z-40 relative pt-[calc(env(safe-area-inset-top)+2.5rem)] pb-4">
      ...
      <nav className="fixed bottom-[calc(env(safe-area-inset-bottom)+1.5rem)] left-1/2 -translate-x-1/2 z-[100] w-[92%] max-w-[420px]">
        <div className="glass-card rounded-full p-2 flex items-center justify-between shadow-[0_40px_80px_rgba(0,0,0,0.8)] border-white/5 bg-zinc-950/90 backdrop-blur-3xl">
          <button onClick={() => navigateTo('dashboard')} className={`flex-1 py-4 rounded-full text-[9px] uppercase tracking-[0.4em] font-black transition-all ${currentView === 'dashboard' ? 'bg-white text-black shadow-lg' : 'text-zinc-700 hover:text-zinc-400'}`}>Core</button>
          <button onClick={() => navigateTo('insights')} className={`flex-1 py-4 rounded-full text-[9px] uppercase tracking-[0.4em] font-black transition-all ${currentView === 'insights' ? 'bg-white text-black shadow-lg' : 'text-zinc-700 hover:text-zinc-400'}`}>Log</button>
          <button onClick={() => navigateTo('flow')} className={`flex-1 py-4 rounded-full text-[9px] uppercase tracking-[0.4em] font-black transition-all ${currentView === 'flow' ? 'bg-white text-black shadow-lg' : 'nothing-red hover:opacity-80'}`}>Flow</button>
